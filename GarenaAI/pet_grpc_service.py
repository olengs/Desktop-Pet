...
class GarenaPetGrpcService(rpc.GarenaPetServiceServicer):
    ...
    async def SendText(self, request: pb.TextRequest, context: grpc.aio.ServicerContext) -> pb.PetResponse:
        player_id = _player_id(request.context.player_id)
        ...
        reply = await self._generate_reply(player_id, message, request.context)
        mood = mood_from_traits(request.context.local_traits)
        return await self._pet_response(request.request_id, reply, mood)

    async def SendVoice(self, request: pb.VoiceRequest, context: grpc.aio.ServicerContext) -> pb.PetResponse:
        player_id = _player_id(request.context.player_id)
        ...
        reply = await self._generate_reply(player_id, transcript, request.context)
        mood = mood_from_traits(request.context.local_traits)
        return await self._pet_response(request.request_id, reply, mood, transcript=transcript)

    async def SendGameEvent(
        self,
        request: pb.GameEventRequest,
        context: grpc.aio.ServicerContext,
    ) -> pb.GameEventReply:
        player_id = _player_id(request.context.player_id)
        game = request.game.strip() or "Garena"
        event_type = request.event_type.strip() or "game_event"
        summary = request.summary.strip() or f"{game} event: {event_type}"
        mood = mood_from_traits(request.impact)

        await self._hub.publish(
            player_id,
            _text_server_message(f"I saved that {game} memory: {summary}", mood),
        )
        return pb.GameEventReply(accepted=True, summary=summary, mood=mood)
    ...
    async def _generate_reply(self, player_id: str, message: str, player_context: pb.PlayerContext) -> str:
        prompt_context = format_player_context(
            player_id,
            player_context.memory_context,
            player_context.local_traits,
        )
        reply = await generate_response(player_id, message, memory_context=prompt_context)
